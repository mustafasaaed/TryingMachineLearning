...
def train_neural_network(x):
    predction = neural_network_model(x)
    cost = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=predction, labels=y))
    ...
    with tf.Session() as sess:
        ...
        correct = tf.equal(tf.arg_max(predction, 1), tf.arg_max(y, 1))
